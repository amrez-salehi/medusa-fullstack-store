...
// A cart can be created before the middleware has assigned a cache-id cookie
// (for example on a direct product deep-link). Next.js rejects an empty cache
// tag, so cache invalidation must be skipped when no tag is available.
...
export async function retrieveCart(cartId?: string, fields?: string) {
  ...
  fields ??=
    "*items, *region, *customer, *items.product, *items.variant, *items.thumbnail, *items.metadata, +items.total, *promotions, +shipping_methods.name"
